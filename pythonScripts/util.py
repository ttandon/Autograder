...
def write(SET, type, vocab1, vocab2, vocab3, inv_index_u, inv_index_b, inv_index_t, data, docs_u, docs_b, docs_t , tfidf_u, tfidf_b, tfidf_t, numFeat, numSiml, leNN):
	...
	ofile ="output/%s_%d.txt" % (type, SET )		# This file is the main file that contains the scores given by both graders, the
	filew = open(ofile, "w")						# predicted score calculated, the number of words, sentences, and average size of sentences.
	##ofile ="%s_%d_f.csv" % (type, SET )	# This file contains the lists that an essay have the answers or not. 
	##filewf = open(ofile, "w")
	##ofile ="%s_%d_s.csv" % (type, SET )	# This file contains the first type of cosine similarities
	##filews = open(ofile, "w")

	##ofile ="%s_%d_s2.csv" % (type, SET )	# This file contains the second type of cosine similarities
	##filews2 = open(ofile, "w")


	ofile ="output/%s_%d_w.txt" % (type, SET )	# This file contains the bag of words
	fileww = open(ofile, "w")
	##ofile ="%s_%d_wt.csv" % (type, SET )	# This file contains the bag of words with tfidf weighting
	##filewwt = open(ofile, "w")
	ofile ="output/%s_%d_b.txt" % (type, SET )	# This file contains the bag of bigrams
	filewb = open(ofile, "w")
	##ofile ="%s_%d_bt.csv" % (type, SET )	# This file contains the bag of bigrams with tfidf weighting
	##filewbt = open(ofile, "w")
	ofile ="output/%s_%d_t.txt" % (type, SET )	# This file contains the bag of trigrams
	filewt = open(ofile, "w")
	##ofile ="%s_%d_tt.csv" % (type, SET )	# This file contains the bag of trigrams with tfidf weighting
	##filewtt = open(ofile, "w")

	##ofile ="%s_%d_n.csv" % (type, SET )	# This file contains the bag of words normalized with tfidf weighting
	##filewn = open(ofile, "w")
	...
	for Id in data:
		...
		##filew.write(" 1:%d " % (essay['f']) )
		if type == 'training': filew.write(" s1:%d s2:%d " % (essay['s1'], essay['s2']) )
		filew.write(" 1:%d 2:%d 3:%f 4:%d" % ( essay['n_words'], essay['n_sentences'], essay['sentence_ave_size'], essay['error_count']) )
	
		counter = 5
		for ai in fs:									
			##filewf.write(" " + counter + ":" + str(ai))
			counter+= 1
		##for ai in lsim:									##filews.write("," + str(ai))
		##for ai in lsim2:								##filews2.write("," + str(ai))
		for word in vocab1:								fileww.write(" " + word + ":" + str( docs_u[Id][word] ) )
		...
		for word in vocab1:								
			...
			##filewn.write("hello")
		for word in vocab2:								filewb.write(" " + word + ":" + str( docs_b[Id][word] ) )
		##for word in vocab2:								##filewbt.write("," + str( tfidf_b[word][Id] ) )
		for word in vocab3:								filewt.write(" " + word + ":" + str( docs_t[Id][word] ) )
